...
fn the_reserved_spellings_are_in_the_vocabulary() {
    assert_eq!(dtype_token(ElementKind::Fp8E4M3FNUZ), "e4m3fnuz");
    assert_eq!(dtype_token(ElementKind::Fp8E5M2FNUZ), "e5m2fnuz");
    ...
    assert_ne!(
        dtype_token(ElementKind::Fp8E4M3),
...
fn a_reserved_dtype_declines_distinctly_from_an_unknown_token() {
    ...
    let reserved_tok = good.replacen("|f32|", "|e4m3fnuz|", 1);
    assert_ne!(
        reserved_tok, good,
    ...
    match StructureKey::parse_token(&reserved_tok) {
        Err(TokenDecline::ReservedDtype { spelling }) => assert_eq!(spelling, "e4m3fnuz"),
        other => panic!(
...
fn a_reserved_dtype_is_refused_inside_the_contraction_precision_group() {
    ...
    let poisoned = format!("{}/e4m3fnuz/{}", &tok[..idx], &tok[idx + 5..]);
