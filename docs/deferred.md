...
| **Publish `unpopped-cpu-c` / `unpopped-slang`** — the reference emitters, currently unreachable to anyone outside this repo | **The `Backend` trait settling** — concretely, the two gaps the crate README still names as open: no structured binding/ABI manifest, and an artifact type that is source text rather than an arbitrary word stream (recorded since `be8a04e`, 2026-08-06 pre-extraction, as "the Vulkane review's #1/#2" — **treat that attribution as unsourced**: Vulkane has no record of raising them and the primary review is not in this repo. **The trigger is the condition, not a party**: these close when a backend that emits a *word stream* rather than source text needs them. No current consumer does — Vulkane consumes nothing from this lineage, and Fuel is still on the pre-extraction `baracuda-kernelgen`. A trigger naming an unsourced party is a trigger nobody watches, which is the artifact-not-event rule already recorded in this file, one level up.). **Ruled 2026-08-15: hold.** The recruiting argument for publishing is real — KISS-EMIT §8.2-0002's freeze gate needs an emitter whose surface spellings differ from the reference's, and a dependable published emitter is what makes an outsider's life easier. But it inverts once you ask what they would be implementing *against*: a third party who adopts `unpopped-slang` and then eats two breaking `Backend` changes in a month concludes the umbrella is not ready, and is right. **Unreachable is a better first impression than unstable.** Note the trigger is an artifact (those two gaps closed), not an event ("when someone asks") — a demand-shaped trigger would fire exactly when the churn is most expensive. |
