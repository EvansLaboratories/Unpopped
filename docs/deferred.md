...
| **`baracuda-cuda-parse`** (new crate, Baracuda-owned, alongside the existing `baracuda-cuda-emit` 0.11.0): the CUDA-specific slice of `convert.rs` — `parse_cuda`, `CUDA_RESIDUE`, the `CUDA` `Frontend` const, and the `tree-sitter-cuda` dep (`crates/unpopped/src/convert.rs:44,66,145`, `crates/unpopped/Cargo.toml:34,60`, measured at `3a787fd`). **Scope corrected 2026-09-25, and the correction shrinks the job**: the 11 private CST walkers and `lift_elementwise`/`lift_reduction`/`lift_scan`/`lift` are shared with Slang (`tree-sitter-slang` is built on `tree-sitter-cpp`, identical node shapes) and stay in neutral core, proven externally reusable by `a_caller_defined_frontend_lifts_through_the_shared_walkers`. `lift.rs` is **not** part of this donation at all — it is generic infra both frontends call and stays in neutral core. **Two separate crates, not one**: this satisfies the old row's "must not be designed emit-only" warning by construction rather than by discipline — `baracuda-cuda-parse` carries `.cu`→IR, `baracuda-cuda-emit` carries IR→`.cu`, and no single crate promises both directions. | ~~The 0.2 trait freeze.~~ **Restated 2026-09-25, artifact-shaped, not version-shaped** — the version-shaped trigger is what went stale the first time (0.2 shipped 2026-08-15 and `Backend`/`Lowering` took two more breaking changes since: 0.4.0's `Result<Spelling, LowerError>`, 0.5.0's `DeclinedOp` split). Unblocks when **both**: **(a)** `baracuda-cuda-parse` exists and passes the byte-match/refusal tests `convert.rs`'s CUDA branch carries today, **and (b)** the KISS registry repoint has landed — §6.8-0003's `cuda` `reference_implementation` moves from `unpopped-vocab` to `baracuda-cuda-vocab` in KISS's `conformance/registry/namespaces.json`. **(b) has not happened yet** and is Baracuda's step, not ours (`crates/unpopped-vocab/src/target.rs:88-90`: *"Sequencing is Baracuda's"*) — do not evict the four `RESERVED` `cuda:` tokens from `target.rs` before it lands; that is the PR-gated breakage this row has always warned about. **Baracuda has conditionally assented to the donation** (via its lane, `si0vfjwv`, 2026-09-25) — conditional on the crate split above (parse separate from emit) and on Baracuda doing the registry repoint itself on its own timeline. |
...
- ~~**The oracle cannot carry an e5m2 NaN's PAYLOAD**~~ — **CLOSED 2026-09-05,
  the same day it was found.** `oracle::eval` now moves RAW bytes for the
  bit-preserving ops (`Neg`/`Abs`/`Copysign` as masks, `Max`/`Min` comparing
  through `f64` and then moving the winner's bytes). `Select` already did.

  **The whole 256-byte domain now agrees between the compiled emitter and the
  oracle, for both fp8 dtypes, with no carve-out** —
  `cpu_end_to_end::a_sign_edit_agrees_with_the_oracle_on_every_fp8_byte`.

  ⚠️ **Kept because of what it cost to notice.** `f64` has ONE NaN class and
  `e5m2` has six encodings, so the oracle's decode-compute-encode detour
  collapsed them — and it had disagreed with a correct emitter on `Max` **since
  `54a256b` in August**. The emitter was right, the reference was wrong, and
  **nothing could detect it, because no test compared the two legs on this
  path.** They agreed while both were broken; fixing the emitter is what turned
  a silent agreement into a visible divergence.

  **The intermediate state is the transferable part.** For one commit the gap was
  BOUNDED rather than excluded: the test asserted every divergence was an e5m2
  NaN case *and that the gap still existed*. **That second assertion is what
  retired it** — the fix made it fire, saying "delete this carve-out", instead of
  leaving a permanently-green exclusion. A carve-out with no expiry condition is
  the thing nobody revisits.

  **Restricted to the narrow floats deliberately**: that is where the loss was
  measured. `f32`/`f64` round-trip their own storage exactly, so widening it
  would change behaviour with nothing to fix.
